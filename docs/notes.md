...
Ground coupling and two temperatures come from `rig.js` at hour 10.2 / elevation 32. Three.js `smoothstep` is `(x, min, max)` — the first meadow used GLSL argument order and baked a perfectly flat plane. Flattening was also keyed to the world origin, which erased every roll in the look corridor toward the ridges; pads are now only under the horse and the village. Density pass uses recipe surfaces, cutout grass and tree cards, a shorter lathed horse with the sky PMREM on hide, displaced Lambert/Basic ridges, and a cheap display-space grade — technique from the recipe, not copied reference games.
